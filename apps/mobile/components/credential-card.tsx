      ...
      <View style={{
        ...
        <View style={{ flex: 1, backgroundColor: '#111827', padding: 16 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
            <Text style={{ color: '#F9FAFB', fontWeight: '600', fontSize: 16 }} numberOfLines={1}>
              {credential.typeName}
            </Text>
            <StatusBadge status={credential.status} />
          </View>
          <Text style={{ color: '#6B7280', fontSize: 12, marginBottom: 12 }} numberOfLines={1}>
            {credential.issuerName}
          </Text>
          {previewClaims.map(([key, value]) => (
            <View key={key} style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 4 }}>
              <Text style={{ color: '#6B7280', fontSize: 12, textTransform: 'capitalize' }}>{key}</Text>
              <Text style={{ color: '#F9FAFB', fontSize: 12 }}>{String(value)}</Text>
            </View>
          ))}
