# TrustiLock — Production Onboarding Guide

## How Organizations Join the TrustiLock Ecosystem

---

## 1. Overview

TrustiLock is an open ecosystem. Any organization can participate in one or more roles:
...
Someone must decide who is trusted. TrustiLock supports multiple governance models:

| Model | Who Decides | Best For | Example |
|---|---|---|---|
| **Platform-run** | TrustiLock operator | Early-stage, single-org deployment | You manually approve issuers |
...
| **Domain ownership** | DNS TXT record verification | Add TXT record `trustilock-verify=<token>` to their DNS |
...
3. KEY MANAGEMENT
   ...
   - Notify TrustiLock governance within 1 hour of key compromise
...
7. LIABILITY
   ...
   - TrustiLock is not liable for issuer's data quality
...
9. AUDIT
   ...
   - TrustiLock may request ad-hoc audit with 30 days notice
```
...
4. TrustiLock resolves the DID to verify
```

**Option B: did:key (For testing/sandbox)**
```
TrustiLock generates a did:key for the issuer during sandbox testing.
Production MUST use did:web.
```

**Key point:** The issuer controls their own keys. TrustiLock NEVER has access to issuer's private keys.
...
Issuer deploys:
  ...
  - Uses TrustiLock's open-source issuer SDK (@trustilock/issuer-sdk)
...
**Model B: Issuer uses TrustiLock as a service (White-label)**
```
Issuer uses TrustiLock's hosted issuer service:
  - TrustiLock provides API: POST /api/v1/issue
  - Issuer sends claim data
  - TrustiLock signs with issuer's key (held in shared HSM partition)
  - TrustiLock manages OID4VCI endpoints

  Note: Issuer's HSM partition is isolated. TrustiLock operators cannot extract keys.
```
...
Test Suite:
  ...
  4. Cross-verify: TrustiLock wallet ↔ issuer endpoint interoperability
...
| Key rotation | Every 24 months max | Issuer rotates keys, notifies TrustiLock |
...
7. AUDIT
   ...
   - TrustiLock may audit data handling practices
```
