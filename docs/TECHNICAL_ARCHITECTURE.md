...
TrustVault is a Turborepo monorepo managed with **pnpm workspaces**.

```
trustvault/
  apps/
    api/            # NestJS modular monolith  (@trustvault/api)
    web/            # Next.js + shadcn/ui portals (issuer, verifier, admin)
    mobile/         # React Native + Expo wallet
  packages/
    shared/         # Cross-app TypeScript types (@trustvault/shared)
  infrastructure/
    docker-compose.yml
    seed/seed-data.ts
```
...
```typescript
{
  ...
  jwt: {
    secret: string,                  // JWT_SECRET || 'trustvault-dev-secret-...'
...
| `JWT_SECRET` | Secret for HS256 auth JWT signing | `trustvault-dev-secret-change-in-production` | **Yes (production)** |
