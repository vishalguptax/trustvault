# TrustVault End-to-End Testing Guide

This guide walks you through testing the entire TrustVault system — backend API, web dashboards, and mobile wallet — working together.
...
|------|-------|----------|-----|
| Admin | `admin@trustvault.dev` | `Admin@123456` | http://localhost:3000/login → redirects to /admin |
| Issuer | `issuer@trustvault.dev` | `Issuer@123456` | http://localhost:3000/login → redirects to /issuer |
| Verifier | `verifier@trustvault.dev` | `Verifier@123456` | http://localhost:3000/login → redirects to /verifier |

### Mobile App (Wallet)

| Role | Email | Password |
|------|-------|----------|
| Holder | `holder@trustvault.dev` | `Holder@123456` |
...
1. Open the TrustVault mobile app on your phone
...
1. Log in as issuer (`issuer@trustvault.dev`)
2. Go to **Credentials** list
3. Find the education credential and click **Revoke**
4. Now switch to verifier (`verifier@trustvault.dev`)
...
1. Log in as admin (`admin@trustvault.dev`)
...
TOKEN=$(curl -s -X POST http://localhost:8000/auth/login \
  ...
  -d '{"email":"issuer@trustvault.dev","password":"Issuer@123456"}' \
...
VERIFIER_TOKEN=$(curl -s -X POST http://localhost:8000/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"verifier@trustvault.dev","password":"Verifier@123456"}' \
  | node -pe "JSON.parse(require('fs').readFileSync(0,'utf8')).data.access_token")
